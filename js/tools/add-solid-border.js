/**
 * Add Solid Border Engine - Client-Side Real Engine
 */
function init_add_solid_border() {
  try {
    ...
    function processImage() {
      ...
      const val_img_format = el_img_format ? (parseFloat(el_img_format.value) || el_img_format.value) : 25;
        ...
        if (loadedImg) {
          ...
          ctx.fillStyle = '#FFFFFF'; ctx.font = '24px sans-serif'; ctx.fillText('Add Solid Border', 50, height / 2);
        }

        let report = `=== ${'Add Solid Border'.toUpperCase()} REPORT ===\nDimensions: ${width} x ${height} px\nStatus: ✅ Canvas Rendered\n`;
        if (out) out.value = report;

        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            containerId: 'gen-results-card',
            title: '✨ Add Solid Border Workspace',
        ...
        if (window.showToast) window.showToast('Add Solid Border processed!', 'success');
      } catch (err) {
      ...
      downloadBtn.addEventListener('click', () => {
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            const a = document.createElement('a'); a.href = url; a.download = 'add-solid-border-output.png'; a.click();
    ...
    console.error('[Engine Error] add-solid-border:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_add_solid_border);
} else {
  init_add_solid_border();
}
