/**
 * IMAGE SEPIA FILTER - Real HTML5 Canvas Image Processor Engine
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderImage() {
    ...
    if (!loadedImg) {
      ...
      ctx.fillText('IMAGE SEPIA FILTER', 300, 180);
    ...
    if ('image-sepia-filter'.includes('rotator')) {
      ...
      ctx.restore();
    } else if ('image-sepia-filter'.includes('flip')) {
      ctx.save();
      ctx.scale(-1, 1);
      ctx.drawImage(loadedImg, -canvas.width, 0);
      ctx.restore();
    } else if ('image-sepia-filter'.includes('border') || 'image-sepia-filter'.includes('padding')) {
      ...
      ctx.fillStyle = 'image-sepia-filter'.includes('transparent') ? 'rgba(0,0,0,0)' : '#3b82f6';
      ...
      for (let i = 0; i < data.length; i += 4) {
        if ('image-sepia-filter'.includes('inverter') || 'image-sepia-filter'.includes('invert')) {
          data[i] = 255 - data[i];
          data[i+1] = 255 - data[i+1];
          data[i+2] = 255 - data[i+2];
        } else if ('image-sepia-filter'.includes('black-and-white') || 'image-sepia-filter'.includes('monochrome')) {
          const gray = 0.299 * data[i] + 0.587 * data[i+1] + 0.114 * data[i+2];
          data[i] = data[i+1] = data[i+2] = gray;
        } else if ('image-sepia-filter'.includes('sepia')) {
          const r = data[i], g = data[i+1], b = data[i+2];
          data[i] = Math.min(255, r * 0.393 + g * 0.769 + b * 0.189);
          data[i+1] = Math.min(255, r * 0.349 + g * 0.686 + b * 0.168);
          data[i+2] = Math.min(255, r * 0.272 + g * 0.534 + b * 0.131);
        } else if ('image-sepia-filter'.includes('brightness') || 'image-sepia-filter'.includes('exposure')) {
          data[i] = Math.min(255, Math.max(0, data[i] + factor));
          data[i+1] = Math.min(255, Math.max(0, data[i+1] + factor));
          data[i+2] = Math.min(255, Math.max(0, data[i+2] + factor));
        } else if ('image-sepia-filter'.includes('threshold')) {
          const gray = 0.299 * data[i] + 0.587 * data[i+1] + 0.114 * data[i+2];
    ...
    if (out) {
      if ('image-sepia-filter'.includes('base64')) {
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', () => {
      ...
      a.href = dataUrl;
      a.download = `image-sepia-filter-output.${ext}`;
