/**
 * Remove Html Tags Engine - Client-Side Real Engine
 */
function init_remove_html_tags() {
  try {
      ...
      try {
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Remove Html Tags Workspace',
            status: status,
            archetype: 'dev',
        ...
        if (window.showToast) window.showToast('Remove Html Tags processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'remove-html-tags-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] remove-html-tags:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_remove_html_tags);
} else {
  init_remove_html_tags();
}
