/**
 * Base64 To Image Engine - Client-Side Real Engine
 */
function init_base64_to_image() {
  try {
      ...
      const val_img_format = el_img_format ? (parseFloat(el_img_format.value) || el_img_format.value) : 25;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Base64 To Image Workspace',
            status: status,
            archetype: 'dev',
        ...
        if (window.showToast) window.showToast('Base64 To Image processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'base64-to-image-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] base64-to-image:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_base64_to_image);
} else {
  init_base64_to_image();
}
