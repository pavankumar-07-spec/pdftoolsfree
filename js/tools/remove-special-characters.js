/**
 * Remove Special Characters Engine - Exact Tool Output
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function transformText() {
    ...
    if ('remove-special-characters'.includes('keep-only-letters')) {
      result = raw.replace(/[^a-zA-Zs]/g, '');
    } else if ('remove-special-characters'.includes('keep-only-numbers')) {
      result = raw.replace(/[^0-9s]/g, '');
    } else if ('remove-special-characters'.includes('remove-special-characters')) {
      result = raw.replace(/[^a-zA-Z0-9s]/g, '');
    } else if ('remove-special-characters'.includes('remove-punctuation')) {
      result = raw.replace(/[.,/#!$%^&*;:{}=-_'~()?"`]/g, '');
    } else if ('remove-special-characters'.includes('remove-emojis')) {
    ...
    if (window.showToast) window.showToast('Remove Special Characters completed!', 'success');
  }
