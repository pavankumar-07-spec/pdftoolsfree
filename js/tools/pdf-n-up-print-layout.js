/**
 * Pdf N Up Print Layout Engine - Client-Side Real Engine
 */
function init_pdf_n_up_print_layout() {
  try {
        ...
        if (fileArrayBuffer && PDFLibObj) {
          ...
          if (window.UIDashboardEngine) {
            window.UIDashboardEngine.render({
              ...
              title: '✨ Pdf N Up Print Layout Workspace',
          ...
          let report = "=== PDF N UP PRINT LAYOUT REPORT ===\n";
          report += `File: ${loadedFile ? loadedFile.name : 'document.pdf'}\nPages: ${maxPages}\n`;
          report += "Status: ✅ Processed client-side locally.\n";
          if (out) out.value = report;
          if (window.showToast) window.showToast('Pdf N Up Print Layout processed successfully!', 'success');
        } else {
          if (out) out.value = "=== PDF N UP PRINT LAYOUT ===\nPlease upload a PDF file above to begin processing.";
        }
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        if (processedPdfBytes) {
          ...
          a.download = `${loadedFile ? loadedFile.name.replace(/\.pdf$/i, '') : 'processed'}-pdf-n-up-print-layout.pdf`;
          a.click(); setTimeout(() => URL.revokeObjectURL(url), 2000);
        }
      });
    }
  } catch (err) {
    console.error('[Engine Error] pdf-n-up-print-layout:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_pdf_n_up_print_layout);
} else {
  init_pdf_n_up_print_layout();
}
