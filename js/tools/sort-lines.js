/**
 * Sort Lines Engine - Client-Side Real Engine
 */
function init_sort_lines() {
  try {
    ...
    function calculate() {
      try {
        ...
        let report = `=== ${'Sort Lines'.toUpperCase()} REPORT ===\n`;
        ...
        if (window.showToast) window.showToast('Sort Lines computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'sort-lines-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] sort-lines:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_sort_lines);
} else {
  init_sort_lines();
}
