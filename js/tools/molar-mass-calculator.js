/**
 * Molar Mass Calculator Engine - Client-Side Real Engine
 */
function init_molar_mass_calculator() {
  try {
    ...
    function calculate() {
      try {
      const el_formula_input = document.getElementById('formula-input');
      const val_formula_input = el_formula_input ? (parseFloat(el_formula_input.value) || el_formula_input.value) : 10;

        const numInputs = Array.from(document.querySelectorAll('input[type="number"], input[type="text"]:not(#main-output)'));
        const vals = numInputs.map(i => parseFloat(i.value)).filter(n => !isNaN(n));

        let res = 0;
        let report = `=== ${'Molar Mass Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Molar Mass Calculator Workspace',
        ...
        if (window.showToast) window.showToast('Molar Mass Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'molar-mass-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] molar-mass-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_molar_mass_calculator);
} else {
  init_molar_mass_calculator();
}
