/**
 * Gpa Predictor Engine - Client-Side Real Engine
 */
function init_gpa_predictor() {
  try {
    ...
    function calculate() {
      ...
      const val_main_input = el_main_input ? (parseFloat(el_main_input.value) || el_main_input.value) : 20;
        ...
        let res = 0;
        let report = `=== ${'Gpa Predictor'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Gpa Predictor computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'gpa-predictor-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] gpa-predictor:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_gpa_predictor);
} else {
  init_gpa_predictor();
}
