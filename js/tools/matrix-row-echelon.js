/**
 * Matrix Row Echelon Engine - Client-Side Real Engine
 */
function init_matrix_row_echelon() {
  try {
    ...
    function calculate() {
      try {
        ...
        let primaryRes = 0;
        let report = `=== ${'Matrix Row Echelon'.toUpperCase()} CALCULATION REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Matrix Row Echelon Workspace',
        ...
        if (window.showToast) window.showToast('Matrix Row Echelon calculated!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'matrix-row-echelon-solution.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] matrix-row-echelon:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_matrix_row_echelon);
} else {
  init_matrix_row_echelon();
}
