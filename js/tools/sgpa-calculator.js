/**
 * SGPA CALCULATOR - Real Calculation Engine
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function calculate() {
    ...
    let resText = '';
    if ('sgpa-calculator'.includes('cgpa-to-percentage')) {
...
Equivalent Grade Class: ${val1 >= 8 ? 'First Class with Distinction' : val1 >= 6.5 ? 'First Class' : 'Second Class'}`;
    } else if ('sgpa-calculator'.includes('sgpa') || 'sgpa-calculator'.includes('gpa')) {
      let totalCredits = 0;
...
Status: ${percentageNeeded >= 40 ? 'Passing Mark Attainable' : 'High Effort Required'}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'sgpa-calculator-report.txt';
