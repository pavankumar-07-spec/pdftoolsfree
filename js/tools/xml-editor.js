/**
 * Xml Editor Engine - Client-Side Real Engine
 */
function init_xml_editor() {
  try {
    ...
    function calculate() {
      try {
        ...
        if (window.showToast) window.showToast('Xml Editor processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'xml-editor-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] xml-editor:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_xml_editor);
} else {
  init_xml_editor();
}
