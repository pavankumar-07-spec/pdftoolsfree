/**
 * World Clock Engine - Client-Side Real Engine
 */
function init_world_clock() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'World Clock'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('World Clock computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'world-clock-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] world-clock:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_world_clock);
} else {
  init_world_clock();
}
