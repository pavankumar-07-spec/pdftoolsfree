/**
 * Kmap Solver Engine - Client-Side Real Engine
 */
function init_kmap_solver() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Kmap Solver'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Kmap Solver computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'kmap-solver-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] kmap-solver:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_kmap_solver);
} else {
  init_kmap_solver();
}
