/**
 * Morse Code Encoder Engine - Client-Side Real Engine
 */
function init_morse_code_encoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Morse Code Encoder'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Morse Code Encoder Workspace',
        ...
        if (window.showToast) window.showToast('Morse Code Encoder computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'morse-code-encoder-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] morse-code-encoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_morse_code_encoder);
} else {
  init_morse_code_encoder();
}
