/**
 * Extract Urls Engine - Client-Side Real Engine
 */
function init_extract_urls() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Extract Urls'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Extract Urls computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'extract-urls-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] extract-urls:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_extract_urls);
} else {
  init_extract_urls();
}
