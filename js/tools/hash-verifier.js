/**
 * Hash Verifier Engine - Client-Side Real Engine
 */
function init_hash_verifier() {
  try {
    ...
    function calculate() {
      try {
        ...
        if (window.showToast) window.showToast('Hash Verifier processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'hash-verifier-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] hash-verifier:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_hash_verifier);
} else {
  init_hash_verifier();
}
