/**
 * Reverse Lines Engine - Client-Side Real Engine
 */
function init_reverse_lines() {
  try {
    ...
    function calculate() {
      try {
        ...
        let report = `=== ${'Reverse Lines'.toUpperCase()} REPORT ===\n`;
        ...
        if (window.showToast) window.showToast('Reverse Lines computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'reverse-lines-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] reverse-lines:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_reverse_lines);
} else {
  init_reverse_lines();
}
