...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function transformText() {
    ...
    if ('remove-emojis'.includes('keep-only-letters')) {
      result = raw.replace(/[^a-zA-Zs]/g, '');
    } else if ('remove-emojis'.includes('keep-only-numbers')) {
      result = raw.replace(/[^0-9s]/g, '');
    } else if ('remove-emojis'.includes('remove-special-characters')) {
      result = raw.replace(/[^a-zA-Z0-9s]/g, '');
    } else if ('remove-emojis'.includes('remove-punctuation')) {
      result = raw.replace(/[.,/#!$%^&*;:{}=-_'~()?"`]/g, '');
    } else if ('remove-emojis'.includes('remove-emojis')) {
    ...
    if (window.showToast) window.showToast('Remove Emojis completed!', 'success');
  }
