/**
 * Jwt Decoder Engine - Client-Side Real Engine
 */
function init_jwt_decoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        if (window.showToast) window.showToast('Jwt Decoder processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'jwt-decoder-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] jwt-decoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_jwt_decoder);
} else {
  init_jwt_decoder();
}
