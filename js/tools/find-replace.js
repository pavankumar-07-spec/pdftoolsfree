/**
 * Find Replace Engine - Client-Side Real Engine
 */
function init_find_replace() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Find Replace'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Find Replace computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'find-replace-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] find-replace:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_find_replace);
} else {
  init_find_replace();
}
