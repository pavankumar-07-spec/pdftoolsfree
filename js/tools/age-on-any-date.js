/**
 * Age On Any Date Engine - Client-Side Real Engine
 */
function init_age_on_any_date() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Age On Any Date'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Age On Any Date Workspace',
        ...
        if (window.showToast) window.showToast('Age On Any Date computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'age-on-any-date-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] age-on-any-date:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_age_on_any_date);
} else {
  init_age_on_any_date();
}
