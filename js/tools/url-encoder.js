/**
 * Url Encoder Engine - Client-Side Real Engine
 */
function init_url_encoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Url Encoder'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Url Encoder computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'url-encoder-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] url-encoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_url_encoder);
} else {
  init_url_encoder();
}
