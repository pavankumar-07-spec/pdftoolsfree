/**
 * Xml Formatter Engine - Client-Side Real Engine
 */
function init_xml_formatter() {
  try {
    ...
    function calculate() {
      try {
        ...
        if (window.showToast) window.showToast('Xml Formatter processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'xml-formatter-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] xml-formatter:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_xml_formatter);
} else {
  init_xml_formatter();
}
