/**
 * Logic Gate Simulator Engine - Client-Side Real Engine
 */
function init_logic_gate_simulator() {
  try {
    ...
    function calculate() {
      try {
      const el_gate_select = document.getElementById('gate-select');
      const val_gate_select = el_gate_select ? (parseFloat(el_gate_select.value) || el_gate_select.value) : 10;

        const numInputs = Array.from(document.querySelectorAll('input[type="number"], input[type="text"]:not(#main-output)'));
        const vals = numInputs.map(i => parseFloat(i.value)).filter(n => !isNaN(n));

        let res = 0;
        let report = `=== ${'Logic Gate Simulator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Logic Gate Simulator Workspace',
        ...
        if (window.showToast) window.showToast('Logic Gate Simulator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'logic-gate-simulator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] logic-gate-simulator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_logic_gate_simulator);
} else {
  init_logic_gate_simulator();
}
