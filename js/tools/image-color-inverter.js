/**
 * Image Color Inverter Engine - Client-Side Real Engine
 */
function init_image_color_inverter() {
  try {
    ...
    function processImage() {
      ...
      const val_img_format = el_img_format ? (parseFloat(el_img_format.value) || el_img_format.value) : 25;
        ...
        if (loadedImg) {
          ...
          ctx.fillStyle = '#FFFFFF'; ctx.font = '24px sans-serif'; ctx.fillText('Image Color Inverter', 50, height / 2);
        }

        let report = `=== ${'Image Color Inverter'.toUpperCase()} REPORT ===\nDimensions: ${width} x ${height} px\nStatus: ✅ Canvas Rendered\n`;
        if (out) out.value = report;

        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            containerId: 'gen-results-card',
            title: '✨ Image Color Inverter Workspace',
        ...
        if (window.showToast) window.showToast('Image Color Inverter processed!', 'success');
      } catch (err) {
      ...
      downloadBtn.addEventListener('click', () => {
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            const a = document.createElement('a'); a.href = url; a.download = 'image-color-inverter-output.png'; a.click();
    ...
    console.error('[Engine Error] image-color-inverter:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_image_color_inverter);
} else {
  init_image_color_inverter();
}
