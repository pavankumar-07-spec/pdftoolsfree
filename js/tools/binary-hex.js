/**
 * Binary Hex Engine - Client-Side Real Engine
 */
function init_binary_hex() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Binary Hex'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Binary Hex computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'binary-hex-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] binary-hex:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_binary_hex);
} else {
  init_binary_hex();
}
