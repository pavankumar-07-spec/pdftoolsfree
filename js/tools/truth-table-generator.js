/**
 * Truth Table Generator Engine - Client-Side Real Engine
 */
function init_truth_table_generator() {
  try {
    ...
    function calculate() {
      try {
      const el_bool_expr_input = document.getElementById('bool-expr-input');
      const val_bool_expr_input = el_bool_expr_input ? (parseFloat(el_bool_expr_input.value) || el_bool_expr_input.value) : 10;

        const numInputs = Array.from(document.querySelectorAll('input[type="number"], input[type="text"]:not(#main-output)'));
        const vals = numInputs.map(i => parseFloat(i.value)).filter(n => !isNaN(n));

        let res = 0;
        let report = `=== ${'Truth Table Generator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Truth Table Generator Workspace',
        ...
        if (window.showToast) window.showToast('Truth Table Generator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'truth-table-generator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] truth-table-generator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_truth_table_generator);
} else {
  init_truth_table_generator();
}
