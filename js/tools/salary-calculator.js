/**
 * Salary Calculator Engine - Client-Side Real Engine
 */
function init_salary_calculator() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Salary Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Salary Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'salary-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] salary-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_salary_calculator);
} else {
  init_salary_calculator();
}
