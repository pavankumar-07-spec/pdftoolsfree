/**
 * Vector Angle Engine - Client-Side Real Engine
 */
function init_vector_angle() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Vector Angle'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Vector Angle computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'vector-angle-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] vector-angle:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_vector_angle);
} else {
  init_vector_angle();
}
