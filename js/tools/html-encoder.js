/**
 * Html Encoder Engine - Client-Side Real Engine
 */
function init_html_encoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        if (window.showToast) window.showToast('Html Encoder processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'html-encoder-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] html-encoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_html_encoder);
} else {
  init_html_encoder();
}
