/**
 * Margin Calculator Engine - Client-Side Real Engine
 */
function init_margin_calculator() {
  try {
    ...
    function calculate() {
      ...
      const val_calc_mode = el_calc_mode ? (parseFloat(el_calc_mode.value) || el_calc_mode.value) : 20;
        ...
        let res = 0;
        let report = `=== ${'Margin Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Margin Calculator Workspace',
        ...
        if (window.showToast) window.showToast('Margin Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'margin-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] margin-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_margin_calculator);
} else {
  init_margin_calculator();
}
