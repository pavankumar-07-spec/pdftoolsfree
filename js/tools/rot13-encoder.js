/**
 * Rot13 Encoder Engine - Client-Side Real Engine
 */
function init_rot13_encoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Rot13 Encoder'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Rot13 Encoder computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'rot13-encoder-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] rot13-encoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_rot13_encoder);
} else {
  init_rot13_encoder();
}
