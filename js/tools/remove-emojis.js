/**
 * Remove Emojis Engine - Client-Side Real Engine
 */
function init_remove_emojis() {
  try {
    ...
    function calculate() {
      ...
      const val_str_input = el_str_input ? (parseFloat(el_str_input.value) || el_str_input.value) : 10;
        ...
        let res = 0;
        let report = `=== ${'Remove Emojis'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Remove Emojis computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'remove-emojis-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] remove-emojis:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_remove_emojis);
} else {
  init_remove_emojis();
}
