/**
 * Shopping List Engine - Client-Side Real Engine
 */
function init_shopping_list() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Shopping List'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Shopping List computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'shopping-list-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] shopping-list:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_shopping_list);
} else {
  init_shopping_list();
}
