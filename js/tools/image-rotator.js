/**
 * Image Rotator Engine - Client-Side Real Engine
 */
function init_image_rotator() {
  try {
    ...
    function processImage() {
      ...
      const val_img_format = el_img_format ? (parseFloat(el_img_format.value) || el_img_format.value) : 25;
        ...
        if (loadedImg) {
          ...
          ctx.fillStyle = '#FFFFFF'; ctx.font = '24px sans-serif'; ctx.fillText('Image Rotator', 50, height / 2);
        }

        let report = `=== ${'Image Rotator'.toUpperCase()} REPORT ===\nDimensions: ${width} x ${height} px\nStatus: ✅ Canvas Rendered\n`;
        if (out) out.value = report;

        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            containerId: 'gen-results-card',
            title: '✨ Image Rotator Workspace',
        ...
        if (window.showToast) window.showToast('Image Rotator processed!', 'success');
      } catch (err) {
      ...
      downloadBtn.addEventListener('click', () => {
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            const a = document.createElement('a'); a.href = url; a.download = 'image-rotator-output.png'; a.click();
    ...
    console.error('[Engine Error] image-rotator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_image_rotator);
} else {
  init_image_rotator();
}
