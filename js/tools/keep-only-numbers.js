/**
 * Keep Only Numbers Engine - Exact Tool Output
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function transformText() {
    ...
    if ('keep-only-numbers'.includes('keep-only-letters')) {
      result = raw.replace(/[^a-zA-Zs]/g, '');
    } else if ('keep-only-numbers'.includes('keep-only-numbers')) {
      result = raw.replace(/[^0-9s]/g, '');
    } else if ('keep-only-numbers'.includes('remove-special-characters')) {
      result = raw.replace(/[^a-zA-Z0-9s]/g, '');
    } else if ('keep-only-numbers'.includes('remove-punctuation')) {
      result = raw.replace(/[.,/#!$%^&*;:{}=-_'~()?"`]/g, '');
    } else if ('keep-only-numbers'.includes('remove-emojis')) {
    ...
    if (window.showToast) window.showToast('Keep Only Numbers completed!', 'success');
  }
