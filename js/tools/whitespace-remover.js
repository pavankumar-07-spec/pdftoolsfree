/**
 * Whitespace Remover Engine - Client-Side Real Engine
 */
function init_whitespace_remover() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Whitespace Remover'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Whitespace Remover Workspace',
        ...
        if (window.showToast) window.showToast('Whitespace Remover computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'whitespace-remover-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] whitespace-remover:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_whitespace_remover);
} else {
  init_whitespace_remover();
}
