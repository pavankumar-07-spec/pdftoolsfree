/**
 * Reverse Text Engine - Client-Side Real Engine
 */
function init_reverse_text() {
  try {
    ...
    function calculate() {
      try {
        ...
        let report = `=== ${'Reverse Text'.toUpperCase()} REPORT ===\n`;
        ...
        if (window.showToast) window.showToast('Reverse Text computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'reverse-text-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] reverse-text:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_reverse_text);
} else {
  init_reverse_text();
}
