/**
 * Tip Calculator Engine - Client-Side Real Engine
 */
function init_tip_calculator() {
  try {
    ...
    function calculate() {
      ...
      const val_calc_mode = el_calc_mode ? (parseFloat(el_calc_mode.value) || el_calc_mode.value) : 20;
        ...
        let res = 0;
        let report = `=== ${'Tip Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Tip Calculator Workspace',
        ...
        if (window.showToast) window.showToast('Tip Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'tip-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] tip-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_tip_calculator);
} else {
  init_tip_calculator();
}
