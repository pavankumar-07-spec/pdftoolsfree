/**
 * Url Decoder Engine - Client-Side Real Engine
 */
function init_url_decoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Url Decoder'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Url Decoder computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'url-decoder-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] url-decoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_url_decoder);
} else {
  init_url_decoder();
}
