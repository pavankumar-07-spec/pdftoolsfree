/**
 * Decimal Hex Engine - Client-Side Real Engine
 */
function init_decimal_hex() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Decimal Hex'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Decimal Hex computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'decimal-hex-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] decimal-hex:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_decimal_hex);
} else {
  init_decimal_hex();
}
