/**
 * Morse Code Decoder Engine - Client-Side Real Engine
 */
function init_morse_code_decoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Morse Code Decoder'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Morse Code Decoder Workspace',
        ...
        if (window.showToast) window.showToast('Morse Code Decoder computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'morse-code-decoder-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] morse-code-decoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_morse_code_decoder);
} else {
  init_morse_code_decoder();
}
