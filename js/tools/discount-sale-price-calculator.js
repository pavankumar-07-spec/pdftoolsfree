/**
 * Discount Sale Price Calculator Engine - Client-Side Real Engine
 */
function init_discount_sale_price_calculator() {
  try {
    ...
    function calculate() {
      ...
      const val_calc_mode = el_calc_mode ? (parseFloat(el_calc_mode.value) || el_calc_mode.value) : 20;
        ...
        let res = 0;
        let report = `=== ${'Discount Sale Price Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Discount Sale Price Calculator Workspace',
        ...
        if (window.showToast) window.showToast('Discount Sale Price Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'discount-sale-price-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] discount-sale-price-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_discount_sale_price_calculator);
} else {
  init_discount_sale_price_calculator();
}
