/**
 * Sip Calculator Engine - Client-Side Real Engine
 */
function init_sip_calculator() {
  try {
    ...
    function calculate() {
      try {
      const el_sip_rate = document.getElementById('sip-rate');
      const val_sip_rate = el_sip_rate ? (parseFloat(el_sip_rate.value) || el_sip_rate.value) : 10;
      const el_sip_years = document.getElementById('sip-years');
      const val_sip_years = el_sip_years ? (parseFloat(el_sip_years.value) || el_sip_years.value) : 15;

        const numInputs = Array.from(document.querySelectorAll('input[type="number"], input[type="text"]:not(#main-output)'));
        const vals = numInputs.map(i => parseFloat(i.value)).filter(n => !isNaN(n));

        let res = 0;
        let report = `=== ${'Sip Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Sip Calculator Workspace',
        ...
        if (window.showToast) window.showToast('Sip Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'sip-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] sip-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_sip_calculator);
} else {
  init_sip_calculator();
}
