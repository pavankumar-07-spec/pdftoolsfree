/**
 * Image Border Frame Adder Engine - Client-Side Real Engine
 */
function init_image_border_frame_adder() {
  try {
    ...
    function processImage() {
      ...
      const val_img_format = el_img_format ? (parseFloat(el_img_format.value) || el_img_format.value) : 25;
        ...
        if (loadedImg) {
          ...
          ctx.fillStyle = '#FFFFFF'; ctx.font = '24px sans-serif'; ctx.fillText('Image Border Frame Adder', 50, height / 2);
        }

        let report = `=== ${'Image Border Frame Adder'.toUpperCase()} REPORT ===\nDimensions: ${width} x ${height} px\nStatus: ✅ Canvas Rendered\n`;
        if (out) out.value = report;

        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            containerId: 'gen-results-card',
            title: '✨ Image Border Frame Adder Workspace',
        ...
        if (window.showToast) window.showToast('Image Border Frame Adder processed!', 'success');
      } catch (err) {
      ...
      downloadBtn.addEventListener('click', () => {
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            const a = document.createElement('a'); a.href = url; a.download = 'image-border-frame-adder-output.png'; a.click();
    ...
    console.error('[Engine Error] image-border-frame-adder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_image_border_frame_adder);
} else {
  init_image_border_frame_adder();
}
