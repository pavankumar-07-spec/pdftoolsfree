/**
 * Ico Extractor Engine - Client-Side Real Engine
 */
function init_ico_extractor() {
  try {
    ...
    function processImage() {
      try {
        ...
        if (loadedImg) {
          ...
          ctx.fillStyle = '#FFFFFF'; ctx.font = '24px sans-serif'; ctx.fillText('Ico Extractor', 50, height / 2);
        }

        let report = `=== ${'Ico Extractor'.toUpperCase()} REPORT ===\nDimensions: ${width} x ${height} px\nStatus: ✅ Canvas Rendered\n`;
        if (out) out.value = report;

        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            containerId: 'gen-results-card',
            title: '✨ Ico Extractor Workspace',
        ...
        if (window.showToast) window.showToast('Ico Extractor processed!', 'success');
      } catch (err) {
      ...
      downloadBtn.addEventListener('click', () => {
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            const a = document.createElement('a'); a.href = url; a.download = 'ico-extractor-output.png'; a.click();
    ...
    console.error('[Engine Error] ico-extractor:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_ico_extractor);
} else {
  init_ico_extractor();
}
