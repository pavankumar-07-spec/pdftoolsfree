/**
 * String Splitter Engine - Client-Side Real Engine
 */
function init_string_splitter() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'String Splitter'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ String Splitter Workspace',
        ...
        if (window.showToast) window.showToast('String Splitter computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'string-splitter-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] string-splitter:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_string_splitter);
} else {
  init_string_splitter();
}
