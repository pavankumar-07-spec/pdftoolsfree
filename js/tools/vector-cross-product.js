/**
 * Vector Cross Product (A × B) Engine
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (inputsContainer && !document.getElementById('v-a')) {
    inputsContainer.innerHTML = `
      ...
      <div class="flex gap-3 mt-4">
        <button id="calc-vec-btn" type="button" class="btn btn-primary flex-1">📐 Compute Vector Cross Product (A × B)</button>
      </div>
    `;
  }
  ...
  function compute() {
    ...
    let res = '==========================================================\n             VECTOR CROSS PRODUCT (A × B)\n==========================================================\nVector A = [' + ax + ', ' + ay + ', ' + az + ']\nVector B = [' + bx + ', ' + by + ', ' + bz + ']\n\nCross Product (A × B) = [' + cx + ', ' + cy + ', ' + cz + ']\n==========================================================';
    if (out) out.value = res;
    if (window.showToast) window.showToast('Cross product calculated!', 'success');
