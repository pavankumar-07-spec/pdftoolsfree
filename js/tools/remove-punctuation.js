/**
 * Remove Punctuation Engine - Exact Tool Output
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function transformText() {
    ...
    if ('remove-punctuation'.includes('keep-only-letters')) {
      result = raw.replace(/[^a-zA-Zs]/g, '');
    } else if ('remove-punctuation'.includes('keep-only-numbers')) {
      result = raw.replace(/[^0-9s]/g, '');
    } else if ('remove-punctuation'.includes('remove-special-characters')) {
      result = raw.replace(/[^a-zA-Z0-9s]/g, '');
    } else if ('remove-punctuation'.includes('remove-punctuation')) {
      result = raw.replace(/[.,/#!$%^&*;:{}=-_'~()?"`]/g, '');
    } else if ('remove-punctuation'.includes('remove-emojis')) {
    ...
    if (window.showToast) window.showToast('Remove Punctuation completed!', 'success');
  }
