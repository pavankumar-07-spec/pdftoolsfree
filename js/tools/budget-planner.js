/**
 * Budget Planner Engine - Client-Side Real Engine
 */
function init_budget_planner() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Budget Planner'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Budget Planner Workspace',
        ...
        if (window.showToast) window.showToast('Budget Planner computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'budget-planner-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] budget-planner:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_budget_planner);
} else {
  init_budget_planner();
}
