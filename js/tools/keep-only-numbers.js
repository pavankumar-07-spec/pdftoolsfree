/**
 * Keep Only Numbers Engine - Client-Side Real Engine
 */
function init_keep_only_numbers() {
  try {
    ...
    function calculate() {
      ...
      const val_str_input = el_str_input ? (parseFloat(el_str_input.value) || el_str_input.value) : 10;
        ...
        let res = 0;
        let report = `=== ${'Keep Only Numbers'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Keep Only Numbers Workspace',
        ...
        if (window.showToast) window.showToast('Keep Only Numbers computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'keep-only-numbers-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] keep-only-numbers:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_keep_only_numbers);
} else {
  init_keep_only_numbers();
}
