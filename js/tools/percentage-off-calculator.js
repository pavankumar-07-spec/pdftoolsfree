/**
 * Percentage Off Calculator Engine - Client-Side Real Engine
 */
function init_percentage_off_calculator() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Percentage Off Calculator'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.UIDashboardEngine) {
          window.UIDashboardEngine.render({
            ...
            title: '✨ Percentage Off Calculator Workspace',
        ...
        if (window.showToast) window.showToast('Percentage Off Calculator computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'percentage-off-calculator-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] percentage-off-calculator:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_percentage_off_calculator);
} else {
  init_percentage_off_calculator();
}
