/**
 * Shuffle Lines Engine - Client-Side Real Engine
 */
function init_shuffle_lines() {
  try {
    ...
    function calculate() {
      try {
        ...
        let report = `=== ${'Shuffle Lines'.toUpperCase()} REPORT ===\n`;
        ...
        if (window.showToast) window.showToast('Shuffle Lines computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'shuffle-lines-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] shuffle-lines:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_shuffle_lines);
} else {
  init_shuffle_lines();
}
