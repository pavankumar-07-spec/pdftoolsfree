/**
 * Goal Setter Engine - Client-Side Real Engine
 */
function init_goal_setter() {
  try {
    ...
    function calculate() {
      try {
        ...
        let res = 0;
        let report = `=== ${'Goal Setter'.toUpperCase()} REPORT ===\n\n`;
        ...
        if (window.showToast) window.showToast('Goal Setter computed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'goal-setter-report.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] goal-setter:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_goal_setter);
} else {
  init_goal_setter();
}
