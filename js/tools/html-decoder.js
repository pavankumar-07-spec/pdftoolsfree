/**
 * Html Decoder Engine - Client-Side Real Engine
 */
function init_html_decoder() {
  try {
    ...
    function calculate() {
      try {
        ...
        if (window.showToast) window.showToast('Html Decoder processed!', 'success');
      } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'html-decoder-output.txt'; a.click();
      });
    }
  } catch (err) {
    console.error('[Engine Error] html-decoder:', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', init_html_decoder);
} else {
  init_html_decoder();
}
